      ...
      <div className="w-full max-w-md rounded-3xl border border-(--app-border) bg-(--app-surface) p-6 shadow-2xl">
        ...
        {onRematch ? (
          <button
            type="button"
            onClick={onRematch}
            className="mt-6 w-full rounded-full border border-(--app-border) bg-(--app-accent) px-4 py-3 text-sm font-medium text-white transition hover:bg-(--app-accent-hover)"
          >
            Play again
          </button>
        ) : (
